...
- **Narrate, don't dump.** After each step, emit a 3–5 line summary: which answer you picked for each question, why, and what the step output is (delete cluster / ranked experiments / E2E result / automation plan). No walls of prose.
- **Complete the work, don't hand it back.** Step 4 defaults to auto-build — create a worktree or branch, implement the minimal change from Q3.1, commit, run E2E, report the result. The "complete the work when you can" rule from the-algorithm applies with extra force here; `auto-algorithm` only defers execution when the session genuinely cannot make the edit (no write access, no shell, wrong surface).
...
## Step 2 — Delete (autonomous)

For each of Q2.1, Q2.2, Q2.3: state the question, state your answer, state the one-line rationale. Branch per the-algorithm's Step 2 branching rules.

**Your Q2.1 is the single biggest delete candidate you can justify from Step 1 + repo context.** Frame it as a cluster (root + dependents) — same rule as the-algorithm. If multiple clusters look comparable, pick the one that most directly removes the Q1.2 gap and flag the runner-up in one line ("runner-up was X; going with Y because Z").

**Your Q2.2 answer is A (fully solves) only if the delete removes the Q1.2 gap entirely.** Otherwise B (shrinks) or C (remains). Err toward B if there's ambiguity — partial deletion still moves the work forward.

**Your Q2.3 answer must check the delete candidate against every Q1.3 primitive.** If it deletes a primitive (C), reject it autonomously and pick a different cluster. Do not delete primitives even in autonomous mode.

**Verdict behaviour:**
- Q2.2 = A + Q2.3 = A → **Delete, don't add.** Skip to Conclusion. This is the highest-value outcome — take it whenever it's genuinely available.
- Q2.2 = B + Q2.3 = A/B → apply the delete, proceed to Step 3 with what survives.
- Q2.2 = C → no delete found; proceed to Step 3 with the full-size request.

Output a Step 2 summary — delete cluster applied (or "none: proceeding with full scope"), what survives, and any primitives the delete skirted close to.

---

## Step 3 — Optimise what remains (autonomous)

Run Q3.1 → Q3.6. Same branching rules as the-algorithm. For every question you pick an answer; you do not ask.

- **Q3.1 simplest version:** pick the smallest class of change (A → E) that plausibly resolves the surviving request. If the surviving scope is genuinely one-line, skip Step 3 entirely and go to Step 4.
- **Q3.2 tradeoffs:** generate 2–4 and pick the one that most favours "works now even if brittle" unless Step 1 explicitly priced robustness higher (e.g. Q1.5 = A Common + Q1.1 goal named reliability). Flag your pick inline: "chosen tradeoff: X — buying works-now over long-term robust because Q1.5 = common and the brittle surface is narrow."
- **Q3.3 cuts:** list 2–3 things you're NOT going to do. Be specific — file paths, feature names, platforms excluded. If nothing comes to mind, loop back to Q3.1; scope is too big.
- **Q3.4 interactions:** describe the flow in 2–3 sentences. Name who calls whom, where state lives, where async boundaries are.
- **Q3.5 workflow variants:** generate 2–4 competing variants, tag each (a) works-now brittle or (b) long-term robust. Pick one, state which and why. Surface any sub-questions (filename format, embed syntax, etc.) with recommended defaults and pick them too — do not swallow decisions silently, but do make them.
- **Q3.6 rank as experiments:** produce the ranked list with Name / Hypothesis / Cheapest test / Signal / Mode for each. Pick the top-ranked item; state which and why.

Output a Step 3 summary — surviving scope, cuts, chosen workflow, top experiment with its hypothesis and mode.
...
- The session genuinely cannot make the edit — tell the user what to do manually, citing Q1.3 primitives + Q3.6 hypothesis + Q3.3 cut-scope, then wait at Q4.1.
...
2. Step 2 ran — simplest version chosen or delete applied.
...
Q2.1=<cluster>, Q2.2=<A|B|C>, Q2.3=<A|B|C>, Q3.1=<A-E>, Q3.6=<experiment #1>, Q4=<built|skipped|deferred>, Q5=<automated|refused>
```
...
>
> **Step 2 (autonomous):**
> Q2.1 → cluster: *redundant product-list fetches in the render path* (root: `fetchProductList` called per render; dependents: the 3 call sites in `components/Product*`)
> Q2.2 → **A** Yes, removing the redundancy drops the round-trip count to one
> Q2.3 → **A** No primitive deleted (the query itself stays; only the redundant calls go)
>
> **Verdict: Delete, don't add.** No caching layer needed. Stopped at Step 2.
>
> ### Auto-decisions trace
> Q2.1=redundant-fetches-cluster, Q2.2=A, Q2.3=A, Q3/Q4/Q5=skipped (delete solved it)
>
> ### Decision
...
- [ ] Step 2 — delete decision narrated with one-line rationale; primitives respected
- [ ] Step 3 — ran only if Step 2 left surviving scope; ranked experiments narrated; sub-questions not swallowed
